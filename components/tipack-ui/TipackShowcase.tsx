...
export default function TipackShowcase({ item }: TipackShowcaseProps) {
  return (
    <div className="group relative w-72 h-96 [perspective:1000px] cursor-pointer">
      {/* 3D Container */}
      <div className="w-full h-full relative transition-all duration-500 [transform-style:preserve-3d] group-hover:[transform:rotateY(180deg)]">
        {/* =======================
                   FRONT SIDE
                   ======================= */}
        <div
        ...
        >
          ...
          <div
            className={`absolute -top-12 -left-12 w-32 h-32 rounded-full opacity-20 blur-xl ${item.color}`}
          />

          {/* Image Container - Styled as a circular window */}
          <div className="relative mt-4 w-48 h-48">
            <div
              className={`absolute inset-0 rounded-full opacity-20 scale-110 ${item.color}`}
            />
