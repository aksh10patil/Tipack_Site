    ...
    >
      ...
      <div className="h-2 w-full absolute -top-2 left-0 right-0 overflow-hidden flex">
        {[...Array(20)].map((_, i) => (
          <div
            key={i}
            className={`w-3 h-3 rotate-45 transform origin-bottom-left translate-y-1.5 ${isDark ? "bg-[#2D2424]" : "bg-white"} border-t-2 border-l-2 border-black`}
          ></div>
        ))}
      </div>
      ...
      <div className="p-8 flex-1 flex flex-col">
        <div className="flex justify-between items-start mb-4">
          <div
            className={`p-3 border-2 border-black ${highlight ? "bg-[#F4D03F]" : "bg-transparent"}`}
          >
            <Icon size={24} className={isDark ? "text-white" : "text-black"} />
          </div>
          {highlight && (
            <Zap className="text-[#F4D03F] fill-[#F4D03F] animate-pulse" />
          )}
        </div>
        ...
        <p
          className={`font-mono text-sm mb-6 ${isDark ? "text-gray-400" : "text-gray-500"}`}
        >
          {desc}
        </p>
